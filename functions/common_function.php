...
function getproducts(){
    ...
    $select_query="Select * from `products` order by rand() LIMIT 0,9"; //limit is used to show the product 0 to 9 in mai page index.php
...
//getting the unique catogary
function get_unique_categories(){
    global $conn;
    //condition to check  isset or not
    if(isset($_GET['category'])){
        $category_id=$_GET['category'];
    $select_query="Select * from `products` where category_id=$category_id"; //limit is used to show the product 0 to 9 in mai page index.php
    $result_query=mysqli_query($conn, $select_query);
...
//getting the unique brands
function get_unique_brands(){
    global $conn;
    //condition to check  isset or not
    if(isset($_GET['brand'])){
        $brand_id=$_GET['brand'];
    $select_query="Select * from `products` where brand_id=$brand_id"; //limit is used to show the product 0 to 9 in mai page index.php
    $result_query=mysqli_query($conn, $select_query);
    $num_of_rows=mysqli_num_rows($result_query);
    if($num_of_rows==0){
...
$num_of_rows=mysqli_num_rows($result_query);
if($num_of_rows==0){
    echo "<h2 class='text-center text-danger'>No product found in this category!</h2>";
}
while($row=mysqli_fetch_assoc($result_query)){
 $product_id=$row['product_id'];
 $product_title=$row['product_title'];
...
if(isset($_GET['add_to_cart'])){
    ...
    // $insert_query=mysqli_query($conn, "INSERT INTO `cart_details` (product_id, ip_address, quantity) 
    // VALUES ($get_product_id, '$get_ip_add', 0)");
